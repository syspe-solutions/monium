...
def post_fork(server, worker):
    """Substitui o Celery Beat: dispara as checagens periódicas (empréstimos
    atrasados, garantias vencendo) sem precisar de broker, worker ou
    scheduler externos.
    ...
    """
    threading.Thread(target=_run_daily_inventory_checks, daemon=True).start()


def _run_daily_inventory_checks():
    ...
    from apps.inventory.tasks import check_expiring_warranties, check_overdue_loans
    ...
    while True:
        ...
        try:
            ...
            logger.exception("Falha na checagem periódica de garantias vencendo")
