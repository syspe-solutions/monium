...
class DepreciationServiceTests(TestCase):
    def setUp(self):
        ...
        self.real_estate_category = RealEstateCategory.objects.create(name="Sede", slug="sede-dep")
    ...
    def _movable_asset(self, category, code="PAT-0001"):
        return MovableAsset.objects.create(
    ...
    def test_returns_none_without_value_or_purchase_date(self):
        acquisition = Acquisition.objects.create(item=self._movable_asset(self.category))
    ...
    def test_returns_non_depreciable_when_category_has_no_useful_life(self):
        acquisition = Acquisition.objects.create(
            item=self._movable_asset(self.category_no_life), value=Decimal("1000.00"), purchase_date=date.today(),
        )
    ...
    def test_returns_non_depreciable_when_category_has_zero_useful_life(self):
        acquisition = Acquisition.objects.create(
            item=self._movable_asset(self.category_zero_life), value=Decimal("1000.00"), purchase_date=date.today(),
        )
        result = calculate_depreciation(acquisition)
        self.assertFalse(result.is_depreciable)
        self.assertEqual(result.current_book_value, Decimal("1000.00"))
    ...
    def test_linear_depreciation_at_half_of_useful_life(self):
        ...
        acquisition = Acquisition.objects.create(
            item=self._movable_asset(self.category), value=Decimal("6000.00"), purchase_date=purchase_date,
        )
    ...
    def test_fully_depreciated_after_useful_life_caps_at_zero(self):
        ...
        acquisition = Acquisition.objects.create(
            item=self._movable_asset(self.category), value=Decimal("1200.00"), purchase_date=purchase_date,
        )
        result = calculate_depreciation(acquisition, as_of=date.today())
    ...
    def test_at_purchase_date_book_value_equals_acquisition_value(self):
        acquisition = Acquisition.objects.create(
            item=self._movable_asset(self.category), value=Decimal("6000.00"), purchase_date=date.today(),
        )
        result = calculate_depreciation(acquisition, as_of=date.today())
    ...
    def test_explicit_useful_life_months_skips_category_lookup(self):
        acquisition = Acquisition.objects.create(
            item=self._movable_asset(self.category_no_life), value=Decimal("1200.00"),
    ...
    def test_future_purchase_date_clamps_elapsed_months_to_zero(self):
        acquisition = Acquisition.objects.create(
            item=self._movable_asset(self.category), value=Decimal("6000.00"),
    ...
    def test_zero_value_acquisition_does_not_raise_division_by_zero(self):
        acquisition = Acquisition.objects.create(
            item=self._movable_asset(self.category), value=Decimal("0.00"),
            purchase_date=date.today() - timedelta(days=30 * 30),
        )
        result = calculate_depreciation(acquisition, as_of=date.today())
        self.assertEqual(result.current_book_value, Decimal("0.00"))
    ...
    def test_item_without_movable_asset_relation_is_not_depreciable(self):
        ...
        acquisition = Acquisition.objects.create(
            item=real_estate, value=Decimal("150000.00"), purchase_date=date.today(),
        )
        result = calculate_depreciation(acquisition, as_of=date.today())
        self.assertFalse(result.is_depreciable)
